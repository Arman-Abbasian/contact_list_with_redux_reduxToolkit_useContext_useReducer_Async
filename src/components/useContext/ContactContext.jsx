...
const ContactContext = ({ name, mobile, id, deleteHandler }) => {
  return (
    <div className="flex justify-between items-center gap-5 border border-primary-4 rounded-sm p-2 w-full">
      <Link to={`/contact/${id}`} className="flex-1 flex items-center gap-3">
        <div className="w-10 h-10 bg-primary-4 rounded-full flex justify-center items-center">
          <BsPerson className="w-6 h-6 text-primary-2" />
        </div>
        <div className="flex flex-col gap-2 justify-start items-start flex-1 overflow-hidden text-xs">
